...
gamma_values = (data 
               .get(["date","hml","BMG"])
               .assign(
                   gamma = compute_rolling_beta(data['BMG'], data['hml']),
            )
            .dropna()
)

gamma_values['hedging portfolio'] = gamma_values['BMG'] - gamma_values['gamma'] * gamma_values['hml']

# re check now rolling beta of hedging portfolio on hml

gamma_values['beta_hml'] = compute_rolling_beta(gamma_values['hml'], gamma_values['hedging portfolio'])

# plot rolling beta of hedging portfolio on hml
plot = (ggplot(gamma_values, aes(x='date', y='beta_hml')) +
        geom_line() +
        labs(title='Rolling Beta of Hedging Portfolio on HML',
             x='Date', y='Beta') +
...
# optimal hedge ratio as the beta of the hml on the hedging portfolio

gamma_values['optimal_hedge_ratio'] = compute_rolling_beta(gamma_values['hml'], gamma_values['hedging portfolio'])

# plot rolling beta of hml on hedging portfolio
plot = (ggplot(gamma_values, aes(x='date', y='optimal_hedge_ratio')) +
        geom_line() +
        labs(title='Rolling Beta of HML on Hedging Portfolio',
             x='Date', y='Beta') +
        theme(axis_text_x=element_text(rotation=45, hjust=1)) +
        scale_x_datetime(breaks=date_breaks('1 year'), labels=date_format('%Y')))
st.pyplot(ggplot.draw(plot))

gamma_values['hedged portfolio'] = gamma_values['hml'] - gamma_values['optimal_hedge_ratio'] * gamma_values['hedging portfolio']


cumulative_returns = (1 + gamma_values[['hml', 'BMG', 'hedged portfolio']]).cumprod() - 1
...
cumulative_returns_long = pd.melt(cumulative_returns, id_vars=['date'], 
                                  value_vars=['hml', 'BMG', 'hedged portfolio'], 
